...
export class VaultPopupListFiltersService {
      ...
      return [
        ...
        ...orgs.map((org) => {
          ...
          if (!org.enabled) {
            ...
            icon = "fa-exclamation-triangle tw-text-danger";
          } else if (
  ...
  folders$: Observable<ChipSelectOption<FolderView>[]> = combineLatest([
    ...
    map((folders) =>
      folders.nestedList.map((f) => this.convertToChipSelectOption(f, "fa-folder")),
    ),
  );
  ...
  collections$: Observable<ChipSelectOption<CollectionView>[]> = combineLatest([
    ...
    map((collections) =>
      collections.nestedList.map((c) => this.convertToChipSelectOption(c, "fa-cube")),
    ),
  );
